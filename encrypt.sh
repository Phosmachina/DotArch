...
EOF
    ...
    chmod +x "$hook_path"
    echo "✅ Hook installed! Unencrypted secrets will now block commits."
}

# Check files (for pre-commit hook)
check_files() {
    local failed=0
    echo "🔍 Scanning for unencrypted secrets..."
    ...
    for file in "${FILES_TO_ENCRYPT[@]}"; do
        ...
        if ! is_encrypted "$file"; then
            echo "❌ SECURITY ERROR: Found unencrypted secret file!"
            echo "   File: $file"
            failed=1
        fi
    done
    ...
    if [ "$failed" -eq 1 ]; then
        echo ""
        echo "⛔ Commit blocked."
        echo "💡 Run './encrypt.sh' to encrypt these files before committing."
        exit 1
    fi
...
encrypt_files() {
    ...
    if [ -z "$vault_arg" ]; then
        echo "ℹ️  No executable '$PASSWORD_SCRIPT' found."
        echo "   You will be prompted for the vault password for each file."
    else
        echo "🔑 Using '$PASSWORD_SCRIPT' for automation."
    fi
    ...
    echo "✨ All present files are encrypted."
}
